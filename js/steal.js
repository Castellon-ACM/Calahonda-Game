...
//  Las botellas MÍTICAS NO se pueden robar bajo ningún concepto.
// =====================================================================
const STEAL_COSTS = { legendary: 300, epic: 150, rare: 75, common: 30 };
const UNROBBABLE  = ['Alcohol 96%']; // botellas que nunca se pueden robar

function stealCostFor(rarity) {
  return STEAL_COSTS[rarity] || STEAL_COSTS.common;
}

function isUnrobbable(name) {
  return UNROBBABLE.indexOf(name) !== -1;
}
...
function renderVisitorInventoryWithSteal(targetUsername, inventory) {
  ...
  if (names.length === 0) {
    ...
    names.forEach(function (name) {
      ...
      const noRob    = isUnrobbable(name);
      const el       = document.createElement('div');
      el.className   = 'inventory-item rarity-' + rarity;

      if (noRob) {
        // Botella mítica — mostrar candado, sin botón de robo
        el.innerHTML =
          ...
          '<div style="font-size:10px;color:#00b4ff;font-weight:700;margin-top:6px;">🔒 Intransferible</div>';
      } else {
        el.innerHTML =
          '<div class="bottle-icon">' + bottleIconMarkup(name) + '</div>' +
          '<div class="bottle-level">Nivel ' + prog.level + '</div>' +
...
async function attemptSteal(targetUsername, bottleName, cost) {
  ...
  // Bloqueo extra por código — aunque el botón no aparece, por seguridad
  if (isUnrobbable(bottleName)) {
    resultEl.textContent = '🔒 Esta botella no se puede robar';
    resultEl.style.color = '#00b4ff';
    return;
  }
  ...
  showStealFlip(won, async function () {
    ...
    try {
      ...
      // Comprobación final servidor — no robar míticas aunque llegue aquí
      if (isUnrobbable(bottleName)) {
        resultEl.textContent = '🔒 Esta botella no se puede robar';
        resultEl.style.color = '#00b4ff';
        return;
